...
def fetch_forecast_7days(lat, lon):
    ...
    if resp.status_code == 200:
        ...
        rad = daily['shortwave_radiation_sum']
...
def adsorption_capacity(T, RH):
    # S-образная изотерма (MOF-303 подобная)
    T_ref = 25.0
    RH_eff = RH * (1 - 0.02 * (T - T_ref))
    if RH_eff < 20:
        return 0.01
    elif RH_eff < 60:
        return 0.01 + 0.03 * (RH_eff - 20) / 40
    else:
        return 0.04 + 0.4 * (RH_eff - 60) / 40
...
class LiteApp(ttk.Window):
    def __init__(self):
        ...
        self.csv_data = None  # данные из CSV клиента
    ...
    def setup_water_tab(self):
        ...
        self.progress_bar = ttk.Progressbar(self.tab_water, variable=self.progress_var, maximum=100, mode='determinate')
    ...
    def setup_water_footprint_tab(self):
        ...
        ttk.Button(btn_frame, text="Рассчитать", command=self.calc_water_footprint).pack(side=tk.LEFT, padx=5)
        ttk.Button(btn_frame, text="Загрузить CSV", command=self.load_csv).pack(side=tk.LEFT, padx=5)
    ...
    def setup_help_tab(self):
        text = (
            ...
            "Биобезопасность:\n"
            "- Рекомендуемая доза УФ-облучения: ≥ 400 Дж/м²\n"
            "- Еженедельная пастеризация при 75 °C (30 мин)\n"
            "- Использовать медицинскую медь для внутренних поверхностей\n\n"
        ...
        try:
            ...
            with PdfPages(pdf_name) as pdf:
                ...
                text = (f"Aerogel-CSP Suite v{VERSION} Отчёт\n\n"
                        ...
                        "Рекомендации по биобезопасности:\n"
                        "- УФ-облучение: ≥ 400 Дж/м²\n"
                        "- Еженедельная пастеризация: 75 °C, 30 мин\n"
                        "- Внутренние поверхности: медицинская медь\n")
    ...
    def calc_thermal(self):
        ...
        sigma = 0.01
        rho_l, rho_v = 1400, 10
        q_crit = 0.15 * 2260 * 1000 * (sigma * 9.81 * (rho_l - rho_v) / rho_v**2)**0.25
        q_actual = Q / A_ht if A_ht > 0 else 0
        if q_actual > q_crit:
            warning = f"\n⚠️ Превышен критический тепловой поток ({q_crit:.0f} Вт/м²)! Риск перегрева чипа."
        else:
            warning = f"\n✅ Запас до кризиса кипения: {(1 - q_actual/q_crit)*100:.0f}%"
    ...
    def calc_water_footprint(self):
        try:
            ...
            cur = float(self.current_water_var.get()) if self.csv_data is None else self.csv_data
        except:
            messagebox.showerror("Ошибка", "Введите числовые значения")
            return
        trad_water = gwh * 1_000_000 * 1.8
        trad_m3 = trad_water / 1000
        saved = trad_m3 + cur
        salt_kg = saved * 0.5  # примерное количество соли
        txt = (f"Традиционное потребление: {trad_m3:,.0f} м³/год\n"
               ...
               f"Масса соли (рассол): {salt_kg:,.0f} кг/год\n"
               f"Рекомендация: использовать как техническую соль или антигололёдный реагент")
    ...
    def load_csv(self):
        ...
        try:
            with open(filepath, newline='', encoding='utf-8') as f:
                ...
                row = next(reader)
                self.csv_data = float(row[1])  # предполагаем, что во втором столбце потребление
                self.current_water_var.set(str(self.csv_data))
                messagebox.showinfo("Загружено", f"Данные клиента загружены: {self.csv_data} м³/год")
        except Exception as e:
...
class SplashScreen:
    def __init__(self, parent, duration=2000):
        ...
        self.splash.configure(bg='#0a192f')

        frame = tk.Frame(self.splash, bg='#0a192f')
        frame.pack(expand=True, fill=tk.BOTH)

        label = tk.Label(frame, text="Aerogel-CSP Suite", font=('Segoe UI', 18, 'bold'),
                         fg='white', bg='#0a192f')
        label.pack(pady=(30, 5))

        version_label = tk.Label(frame, text=f"Версия {VERSION}", font=('Segoe UI', 10),
                                 fg='#64b5f6', bg='#0a192f')
        version_label.pack()

        self.progress = ttk.Progressbar(frame, length=250, mode='indeterminate', bootstyle="info-striped")
